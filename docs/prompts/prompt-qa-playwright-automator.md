...
## 🌐 API Testing Playwright (Regras Específicas)

- **SEMPRE** utilize o objeto `request` (APIRequestContext) nativo do `@playwright/test`.
- **NUNCA** utilize bibliotecas externas como Axios, Supertest ou node-fetch.
- **Asserções de Status**: Utilize `expect(response.ok()).toBeTruthy()` ou valide o código exato com `expect(response.status()).toBe(200)`.
- **Validação de Payload**: Extraia o corpo da resposta através do método assíncrono `await response.json()` e valide propriedades pontuais.
- **Autenticação e Headers**: Garanta que Tokens ou Cookies de autenticação sejam passados corretamente na propriedade `headers` (ex: `headers: { Cookie: "token=" + authToken }`).
- **Limpeza de Dados (Teardown)**: Testes de mutação (POST/PUT) devem armazenar o ID criado e garantir um `request.delete()` no final do fluxo para não poluir o ambiente.